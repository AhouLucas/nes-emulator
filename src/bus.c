...
uint8_t Bus_read_prg_rom(Bus* bus, uint16_t addr) {
    addr -= 0x8000;
...
uint8_t Bus_mem_read_u8(Bus* bus, uint16_t addr) {
...
void Bus_mem_write_u8(Bus* bus, uint16_t addr, uint8_t data) {
    uint16_t mirror_down_addr;
    switch (addr) {
        case RAM ... RAM_MIRRORS_END:
            mirror_down_addr = addr & 0b0000011111111111;
...
uint16_t Bus_mem_read_u16(Bus* bus, uint16_t addr) {
...
void Bus_mem_write_u16(Bus* bus, uint16_t addr, uint16_t data) {
